<?php
/**
 * class Shared_Model_Data_SupplyProductionProject
 * 製造加工委託
 * @package Shared
 * @subpackage Shared_Model_Data
 */
class Shared_Model_Data_SupplyProductionProject extends Shared_Model_Data_DbAbstract
{
    protected $_tableName = 'frs_supply_production_project';
		...
		if (!empty($data)) {
			...
			if ($lastDate == $year) {
				...
				if ($lastCount >= 9999) {
					...
					return 'SI' . $year . $nextAlphabet . '0001';
				} 
				return 'SI' . $year . $lastAlphabet . sprintf('%04d', $lastCount + 1);
			}
		}
		
		return 'SI' . $year . '0' . '0001';
    }
}
