<?php
/**
 * class Shared_Model_Pdf_EstimateType1
 * PDF生成 見積書1
 *
 * @package Shared
...
{    
    ...
    public static function makeContent($pdf, $estimateData, $versionData, $companyData)
    {
        ...
        $pdf->MultiCell( 40, $maxCount * 18, $versionData['labels']['label_1'], $cellBorder, 'L', false, 0, '', '', true, 0); // No.
        $pdf->MultiCell(280, $maxCount * 18, $versionData['labels']['label_2'], $cellBorder, 'L', false, 0, '', '', true, 0); // 項目名
        $pdf->MultiCell( 70, $maxCount * 18, $versionData['labels']['label_3'], $cellBorder, 'C', false, 0, '', '', true, 0); // 注文単位
        $pdf->MultiCell(  0, $maxCount * 18, $versionData['labels']['label_4'], $cellBorderLast,  'C', false, 1, '', '', true, 0); // 見積単価／個（税別）
		...
		foreach ($versionData['item_list'] as $each) {
			...
			$height0 = $pdf2->getY();
			$pdf2->MultiCell(280, 0, $each['item_name'] . "\n" . $settingName, $cell1, 'L', false, 1, '', '', true, 0);
			$height1 = $pdf2->getY();

			$height2 = $pdf2->getY();
			$pdf2->MultiCell(150, 0, $each['unit_price'], $cell1, 'L', false, 1, '', '', true, 0);
			$height3 = $pdf2->getY();
			...
			if ($height1 - $height0 < $height3 - $height2) {
				$rowHeight = $height3 - $height2;
			}
			
			
	        $pdf->MultiCell( 40, $rowHeight, $each['id'], $cellBorder, 'L', false, 0, '', '', true, 0);
	        $pdf->MultiCell(280, $rowHeight, $each['item_name'], $cellBorder, 'L', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 70, $rowHeight, $each['unit'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell(150, $rowHeight, $each['unit_price'], $cellBorderLast,  'R', false, 1, '', '', true, 0);
		}
