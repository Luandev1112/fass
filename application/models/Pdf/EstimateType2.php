<?php
/**
 * class Shared_Model_Pdf_EstimateType2
 * PDF生成 見積書2
 *
 * @package Shared
...
{    
    ...
    public static function makeContent($pdf, $estimateData, $versionData, $companyData)
    {
        ...
        $pdf->MultiCell(190, $maxCount * 16, $versionData['labels']['label_2'], $cellBorder, 'L', false, 0, '', '', true, 0); // 項目名
        $pdf->MultiCell( 60, $maxCount * 16, $versionData['labels']['label_3'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税込）
        $pdf->MultiCell( 60, $maxCount * 16, $versionData['labels']['label_4'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税別）
        $pdf->MultiCell( 60, $maxCount * 16, $versionData['labels']['label_5'], $cellBorder, 'C', false, 0, '', '', true, 0); // 仕入掛率
        $pdf->MultiCell( 60, $maxCount * 16, $versionData['labels']['label_6'], $cellBorder, 'C', false, 0, '', '', true, 0); // 項目名
        $pdf->MultiCell(  0, $maxCount * 16, $versionData['labels']['label_7'], $cellBorderLast,  'C', false, 1, '', '', true, 0); // 見積単価／個（税別）


		$pdf2 = clone $pdf;
		$pdf2->setCellPaddings(5,5,5,5);
		$pdf2->AddPage();
		$pdf2->SetFont('ipagp', '', 8);

		foreach ($versionData['item_list'] as $each) {
			$height00 = $pdf2->getY();
			$pdf2->MultiCell(190, 0, $each['item_name'], $cellBorder, 'L', false, 1, '', '', true, 0);
			$height11 = $pdf2->getY();
	        ...
	        $pdf->MultiCell( 40, $height11 - $height00, $each['id'], $cellBorder, 'L', false, 0, '', '', true, 0);
	        $pdf->MultiCell(190, $height11 - $height00, $each['item_name'], $cellBorder, 'L', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height11 - $height00, $each['standard_price_tax'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height11 - $height00, $each['standard_price'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height11 - $height00, $each['unit'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height11 - $height00, $each['wholesale_rate'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell(  0, $height11 - $height00, $each['unit_price'], $cellBorderLast,  'R', false, 1, '', '', true, 0);
		}
