<?php
/**
 * class Shared_Model_Pdf_EstimateType4
 * PDF生成 見積書4
 *
 * @package Shared
...
{    
    ...
    public static function makeContent($pdf, $estimateData, $versionData, $companyData)
    {
		...
		$maxCount = max(array(
			...
			substr_count($versionData['labels']['label_6'], "\n"),
			substr_count($versionData['labels']['label_7'], "\n"),
			substr_count($versionData['labels']['label_8'], "\n"),
		)) + 1;
			
        $pdf->MultiCell( 25, $maxCount * 12, $versionData['labels']['label_1'], $cellBorder, 'L', false, 0, '', '', true, 0); // No.
        $pdf->MultiCell(140, $maxCount * 12, $versionData['labels']['label_2'], $cellBorder, 'L', false, 0, '', '', true, 0); // 項目名
        $pdf->MultiCell( 75, $maxCount * 12, $versionData['labels']['label_3'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税込）
        $pdf->MultiCell( 60, $maxCount * 12, $versionData['labels']['label_4'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税別）
        $pdf->MultiCell( 60, $maxCount * 12, $versionData['labels']['label_5'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税別）
        $pdf->MultiCell( 50, $maxCount * 12, $versionData['labels']['label_6'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税別）
        $pdf->MultiCell( 60, $maxCount * 12, $versionData['labels']['label_7'], $cellBorder, 'C', false, 0, '', '', true, 0); // 参考小売価格（税別）
        $pdf->MultiCell(  0, $maxCount * 12, $versionData['labels']['label_8'], $cellBorderLast, 'C', false, 1, '', '', true, 0); // 仕入掛率
		
		foreach ($versionData['item_list'] as $each) {
			/*$maxCount = max(array(
				substr_count($each['id'], "\n"),
				substr_count($each['item_name'], "\n"),
				substr_count($each['standard_price_import_currency'], "\n"),
				substr_count($each['standard_price_import'], "\n"),
				substr_count($each['standard_price'], "\n"),
				substr_count($each['unit'], "\n"),
				substr_count($each['wholesale_rate_overseas'], "\n"),
				substr_count($each['unit_price'], "\n"),
			)) + 1;*/

			$height0 = $pdf2->getY();
			$pdf2->MultiCell(140, 0, $each['item_name'] . "\n" . $settingName, $cell1, 'L', false, 1, '', '', true, 0);
			$height1 = $pdf2->getY();
	        ...
	        $pdf->MultiCell( 25, $height1 - $height0, $each['id'], $cellBorder, 'L', false, 0, '', '', true, 0);
	        $pdf->MultiCell(140, $height1 - $height0, $each['item_name'], $cellBorder, 'L', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 75, $height1 - $height0, $each['standard_price_import_currency'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height1 - $height0, $each['standard_price_import'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height1 - $height0, $each['standard_price'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 50, $height1 - $height0, $each['unit'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell( 60, $height1 - $height0, $each['wholesale_rate_overseas'], $cellBorder, 'R', false, 0, '', '', true, 0);
	        $pdf->MultiCell(  0, $height1 - $height0, $each['unit_price'], $cellBorderLast,  'R', false, 1, '', '', true, 0);
		}
