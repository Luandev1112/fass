<?php
/**
 * class Shared_Model_Resource_TemporaryPrivate
 *
 * 画像等の仮保存用リソースマネージャ(非公開ディレクトリ)
 *
 * @package Shared
 * @subpackage Shared_Model_Resource
 */
class Shared_Model_Resource_TemporaryPrivate extends Shared_Model_Resource_Abstract
{
...
//-------------------------------------------------------------------------------------
     ...
     * @return string
     */
    public static function isExist($fileNameWithExtension)
    {
        if (!file_exists(self::getResourceObjectPath($fileNameWithExtension))) {
        	return false;
        }
...
//-------------------------------------------------------------------------------------
    ...
    {
        return self::_getResourcePath('temp' . DIRECTORY_SEPARATOR . $fileNameWithExtension);
    }
