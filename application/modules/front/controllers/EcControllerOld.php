...
class EcController extends Front_Model_Controller
{
    ...
    {  
        ...
        $selectObj = $itemTable->select();
        $selectObj->where('frs_item.status != ?', Shared_Model_Code::ITEM_STATUS_REMOVE);
        $selectObj->where('frs_item.item_type = ?', Shared_Model_Code::ITEM_TYPE_BUNDLE);
        $selectObj->order('frs_item.id ASC');
        ...
        $selectObj = $itemTable->select();
        $selectObj->where('frs_item.status != ?', Shared_Model_Code::ITEM_STATUS_REMOVE);
        $selectObj->where('frs_item.item_type = ?', Shared_Model_Code::ITEM_TYPE_PACKAGE);
        $selectObj->order('frs_item.id ASC');
	            ...
	            try {
					...
					foreach ($oldBundleList as $eachOldBundle) {
						...
						if ($isExist == false) {
			            	...
			            	$packageBundleTable->updateById($eachOldBundle['id'], $params);
						}
					}
		            ...
		            if (!empty($packageBundleIdList)) {
			            foreach ($packageBundleIdList as $eachPackageBundleId) {
			            	...
			            	if (is_numeric($eachPackageBundleId) && $packageBundleTable->isExist($eachPackageBundleId)) {
				            	...
				            	$packageBundleTable->updateById($eachPackageBundleId, $params);
			            	} else {
				                $params = array(
							        'status'               => Shared_Model_Code::ITEM_CODE_BUNDLE_STATUS_ACTIVE,
							        'item_package_id'      => $packageId,
